...
!(async () => {
    if ($.isNode()) {
        //$.isNode()环境执行部分  青龙执行
        if (!process.env.wzl_dachao_data) {
            console.log(`\n【${$.name}】：未填写相应变量 wzl_dachao_data`);
            return;
        }
        if (process.env.wzl_dachao_data && process.env.wzl_dachao_data.indexOf('@') > -1) {
            app_wzl_dachao_data = process.env.wzl_dachao_data.split('@');
        }else {
            app_wzl_dachao_data = process.env.wzl_dachao_data.split();
        }
    }

    console.log(`-------- 共 ${app_wzl_dachao_data.length} 个账号 --------`)
    // console.log(app_wzl_dachao_data)
    console.log(
    ...
    for (i = 0; i < app_wzl_dachao_data.length; i++) {
        $.index = i + 1;
        console.log(`\n====== 开始【第 ${$.index} 个账号】======`)
        // console.log(`这里是分割后:${app_wzl_dachao_data}`);
        data = app_wzl_dachao_data[i].split('&');
        // console.log(`====== ${data}`)
        user = data[0]
...
function syttdl(timeout = 0) {
    urldl = `https://mapi-plus.hndachao.cn/api/v1/member-center/auth/signIn?system_version=7.1.2&client_id_android=c36a684e113aab0d193243413d716d22&locating_city=海宁&appkey=fd78c26575ccc9ab1f71cf99291011b3&appid=m2okkxdfiqgt93vctq&package_name=com.hoge.android.app.dachao&app_version=13.2.2&client_type=android&version=13.2.2&device_token=cff33ed3f26e79f4e9ac647852c15c44&phone_models=P40&language=Chinese&location_city=海宁`
    // console.log(urldl)
    return new Promise((resolve) => {
        let url = {
            url: urldl,
            headers: {
                'user-agent': "Dalvik/2.1.0 (Linux; U; Android 7.1.2; P40 Build/N2G47H) m2oSmartCity_465 1.0.0",
                'x-device-id': 'cff33ed3f26e79f4e9ac647852c15c44'
            } ,
            body: `password=${pwd}&account=${user}`,
        }
        // console.log(url);
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if (result.error_code == 0) {
                    ...
                    $.log(`\n【🎉🎉🎉 恭喜您鸭 🎉🎉🎉】登录状态: ${JSON.stringify(result.result)} ✅ `)
                    // await $.wait(3 * 1000)
                }else {
                    $.log(`\n【🎉 恭喜个屁 🎉】登录状态:${result.error_message} `)
                }
                access_token = result.result.access_token
                console.log(`这是你的用户id：access_token=${access_token}`)
            } catch (e) {
                $.logErr(e, resp);
            } finally {
                resolve()
